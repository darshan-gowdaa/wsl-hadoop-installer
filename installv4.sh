...
# Hadoop Ecosystem - Interactive Menu Installer v4 (Platform-Aware)
...
select_platform() {
    ...
    echo ""
    echo -e "${CYAN}════════════════════════════════════════════════════════════════${NC}"
    echo -e "${BOLD}${MAGENTA}         Hadoop Ecosystem Installer v4 - Setup              ${NC}"
    ...
    if [ "$detected" = "wsl" ]; then
        echo -e "  ${BOLD}${CYAN}1)${NC} WSL (Windows Subsystem for Linux)  ${GREEN}← detected${NC}"
    else
        echo -e "  ${BOLD}${CYAN}1)${NC} WSL (Windows Subsystem for Linux)"
    fi
    if [ "$detected" = "linux" ]; then
        echo -e "  ${BOLD}${CYAN}2)${NC} Native Linux (Ubuntu / Debian / etc.)  ${GREEN}← detected${NC}"
    else
        echo -e "  ${BOLD}${CYAN}2)${NC} Native Linux (Ubuntu / Debian / etc.)"
    fi
    echo ""

    while true; do
        read -p "  Enter choice [1/2] (default: auto-detected): " pchoice
        case "$pchoice" in
            1) PLATFORM="wsl"; break ;;
            2) PLATFORM="linux"; break ;;
            "")
                PLATFORM="$detected"
                break
                ;;
    ...
    if [ "$PLATFORM" = "wsl" ]; then
        success "Platform set to: WSL"
    else
        success "Platform set to: Native Linux"
    fi
    sleep 1
}
...
preflight_checks() {
    local platform_label="Linux"
    [ "$PLATFORM" = "wsl" ] && platform_label="WSL"
    
    echo -e "\n${CYAN}════════════════════════════════════════════════════════════════${NC}"
    echo -e "${BOLD}${MAGENTA}       Hadoop Ecosystem Installer v4  [${platform_label}]           ${NC}"
    echo -e "${BLUE}               github.com/darshan-gowdaa                ${NC}"
    echo -e "${CYAN}════════════════════════════════════════════════════════════════${NC}\n"
    ...
    # WSL-only: Configure DNS with intelligent fallback
    if [ "$PLATFORM" = "wsl" ]; then
    ...
    if [ "$PLATFORM" = "wsl" ]; then
        ...
        # Check location (WSL-specific: /mnt/ is Windows filesystem)
...
install_eclipse() {
    ...
    # Systemd check - platform-aware
    if [ "$PLATFORM" = "wsl" ]; then
        # WSL: Check if systemd is running, enable if needed
...
WSLCONF'
        ...
        # Native Linux: systemd should be available
        if ! systemctl is-system-running &>/dev/null; then
            error "Systemd is not running. Eclipse requires systemd for snap. Please ensure your system uses systemd."
        fi
    fi
...
show_menu() {
    ...
    echo -e "${CYAN}════════════════════════════════════════════════════════════════${NC}"
    echo -e "  ${BOLD}${MAGENTA}  Hadoop Ecosystem Installer v4${NC} ${YELLOW}[${platform_label}]${NC}  ${BLUE}github.com/darshan-gowdaa${NC}"
    echo -e "${CYAN}════════════════════════════════════════════════════════════════${NC}"
    echo ""
